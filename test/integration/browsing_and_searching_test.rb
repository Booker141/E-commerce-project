...
class BrowsingAndSearchingTest < ActionDispatch::IntegrationTest
 fixtures :producers, :suppliers, :liquors, :liquors_suppliers
  ...
  module BrowsingTestDSL
    ...
    def liquor_details(name)
      @liquor = Liquor.where(:name => name).first
      get "/catalog/show/#{@liquor.id}"
      assert_response :success
      assert_template 'catalog/show'
      assert_select 'div#content' do
        assert_select 'h1', @liquor.name
        assert_select 'h2', "por #{@liquor.producer.name}"
      end
    end
